...
double bspLine::signDistance( const Vector2d &p ) const {
	return (p.x*n.x+p.y*n.y-d);
}

// signed intersection of line with an edge
Vector2d bspLine::intersectLine(const Vector2d &v1,const Vector2d &v2) const {
...
//*****************************************************************************
// bspEdge
bspTree* bspEdge::btree;

bspEdge::bspEdge(bspTree * tree, const int v0, const int v1) { 
	btree = tree;
	e0 = v0; e1 = v1;
...
Vector2d* bspEdge::getVertex(int id) const {
	if (id == 0) {
...
int bspEdge::checkAgainstLine(const bspLine &line)
{
...
int bspEdge::getVertexId(int id) const {
	if (id == 0) { return e0; } else { return e1; }
}

void bspEdge::addEdge(const int v0, const int v1) { 
	e0 = v0; e1 = v1; 
	Vector2d* p0 = btree->getVertex(e0);
	Vector2d* p1 = btree->getVertex(e1);
...
bspEdge* bspNode::getEdge(int id) { 
	return btree->getEdge(evec[id]); 
}

Vector2d bspNode::computeBarycenter() {
	Vector2d sum(0,0);
	double count = 0;
	int ne = (int)evec.size();
	for(int it = 0; it < ne; ++it)
	{
		bspEdge * actual = btree->getEdge(evec[it]);
...
void bspNode::intersect(Vector2d &v1, Vector2d &v2, const bspLine &line, int &i) {  
...
{	   
	...
	{
		if (evec[i] != sedge) {
			bspEdge * ei = btree->getEdge(evec[i]);
	...
	{ 
		...
		{	
			...
			bspEdge *ef = new bspEdge(btree); 
			bspEdge *eb = new bspEdge(btree);
...
{
	delete root;
	for( std::vector<bspEdge*>::const_iterator it = edges.begin(); it != edges.end(); ++it) {
		delete *it;
	}
}
...
void bspTree::initEnvironment()
{
	...
	v[7]  = addVertex(3.5, -5.5);
	v[8]  = addVertex(3.5, -0.5);
	v[9] = addVertex(9.5, -0.5);
	v[10] = addVertex(9.5, 9.5);
	v[11] = addVertex(-0.5, 9.5);
	v[12] = addVertex(-0.5, 3.5);
	v[13] = addVertex(-5.5, 3.5);
	...
	bspEdge * e;
	for(int i = 0; i < 19; ++i) {
		e = new bspEdge(this, v[i], v[i+1]); p = addEdge(e); root->addEdge(p);
	}
	e = new bspEdge(this, v[19], v[0]); p = addEdge(e); root->addEdge(p);
	for(int i = 20; i < 23; ++i) {
		e = new bspEdge(this, v[i], v[i+1]); p = addEdge(e); root->addEdge(p); 
	}
	e = new bspEdge(this, v[23], v[20]); p = addEdge(e); root->addEdge(p); 
	...
	e = new bspEdge(this, v[3], v[0]); p = addEdge(e); root->addEdge(p);
	for(int i = 4; i < 9; ++i) {
		e = new bspEdge(this, v[i], v[i+1]); p = addEdge(e); root->addEdge(p); 
	}
	e = new bspEdge(this, v[9], v[4]); p = addEdge(e); root->addEdge(p); 

	for(int i = 10; i < 15; ++i) {
		e = new bspEdge(this, v[i], v[i+1]); p = addEdge(e); root->addEdge(p); 
	}
	e = new bspEdge(this, v[15], v[10]); p = addEdge(e); root->addEdge(p); 
	*/
}
...
void bspTree::query(const normalDist& nd, std::multimap<double, int>& nbrs)
{
	...
	Vector2d p(nd.x, nd.y);
	Vector2d pt(sxinv*(cphi*nd.x+sphi*nd.y), syinv*(-sphi*nd.x+cphi*nd.y));
	...
	while (!Q.empty()) 
	{
		...
		bspEdge * e = getEdge(node->getSplitEdge());
	...
	for(std::multimap<double, int>::iterator it = nbrs.begin(); it != nbrs.end(); ++it) {
		...
		bspEdge * e = getEdge(it->second);
		Vector2d* p0 = getVertex(e->getVertexId(0));
		Vector2d* p1 = getVertex(e->getVertexId(1));
...
void bspTree::testQuery() 
{
	...
	nd.x = 0.06; nd.y = 3.11622;
	nd.M(0,0) = 0.265071; nd.M(0,1) = 0.0708107;
	nd.M(1,0) = 0.0708107; nd.M(1,1) = 0.0281007;
